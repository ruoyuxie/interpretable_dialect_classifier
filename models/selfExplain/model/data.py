...
class ClassificationData(pl.LightningDataModule):
    def __init__(self, basedir: str, tokenizer_name: str, batch_size: int=4, num_workers: int = 1):
        super().__init__()
        self.basedir = basedir
        self.batch_size = batch_size
        self.num_workers = num_workers
        self.num_unique_labels = 0
        ...
        elif tokenizer_name == "xlm-roberta-base":
            self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_name, do_lower_case=True)
    ...
    def train_dataloader(self):
        ...
        self.num_unique_labels = dataset.num_unique_labels
    ...
    def test_dataloader(self):
        dataset = ClassificationDataset(tokenizer=self.tokenizer,
                                        data_path=f"{self.basedir}/all.json")
        return DataLoader(dataset=dataset, batch_size=self.batch_size,
                          shuffle=False, num_workers=self.num_workers, collate_fn=self.collator)
...
class ClassificationDataset(Dataset):
    ...
    def read_dataset(self):
        ...
        self.num_unique_labels = len(set(self.answer_labels))
...
if __name__ == "__main__":
    ...
    dm = ClassificationData(
        basedir=sys.argv[1], model_name=sys.argv[2])
