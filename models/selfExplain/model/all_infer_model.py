...
from transformers import AutoModel, AutoConfig, RobertaConfig, RobertaModel, AutoModelForMaskedLM


def load_model(ckpt, batch_size):

    # load model as xlm-roberta-base instead of checkpoint
    model = AutoModelForMaskedLM.from_pretrained("xlm-roberta-base")
    # maintain the same config as the original model
    config = AutoConfig.from_pretrained("xlm-roberta-base")
    config.d_model = config.hidden_size
    config.dropout = 0.2

    trainer = Trainer(gpus=1)
    dm = ClassificationData(basedir="/scratch/rxie/selfexplain/my-self-exp/evluation/zh/zh", tokenizer_name="xlm-roberta-base",
...
def eval(model, dataloader, concept_map, dev_file, paths_output_loc: str = None):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, total=len(dataloader)):
            ...
            # make predicted_labels all 0s
            # predicted_labels.extend([0] * len(batch_predicted_labels.tolist()))

            # print the probabilities of the predicted labels
            # print(torch.softmax(logits, dim=1).tolist())
