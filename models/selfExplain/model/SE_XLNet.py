...
class SEXLNet(LightningModule):
    def __init__(self, hparams):
        ...
        if self.hparams.model_name == "xlm-roberta-base":
            self.save_hyperparameters(hparams)
            self.save_hyperparameters()

            self.model = AutoModelForMaskedLM.from_pretrained(self.hparams.model_name)
        ...
        elif self.hparams.model_name == "xlnet-base-cased":
    ...
    def forward(self, batch):
        global sentence_cls, hidden_state
        ...
        # make predicted_labels to 0s
        # predicted_labels = torch.zeros_like(predicted_labels)
