...
def read_words(filename):
    ...
    with open(filename, newline='',encoding="utf8") as f:
        ...
        for item in words:
            if len(item.split("\t")) == 2:
                class_zero_word, class_one_word = item.split("\t")
                class_zero_words.append(class_zero_word)
                class_one_words.append(class_one_word)
            elif len(item.split("\t")) == 1:
                class_zero_word = item
                class_zero_words.append(class_zero_word)
            else:
                print("Error: item.split('\t') has more than 2 elements")
